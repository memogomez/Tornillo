...
public class PDFFactura {
	...
	public Document construirPdf(Comprobante comprobante, String selloDigital, byte[] bytesQRCode)
		...
		construirBoceto(comprobante, tfd);

		// si no hay timbre lanzar una excepción
		/*
		 * if (tfd == null) throw CFDIImpresoException
		 */
		/*Font fontTituloSellos = new Font(Font.FontFamily.HELVETICA, 7.5F, Font.BOLD);
		Font fontContenidoSellos = new Font(Font.FontFamily.HELVETICA, 7.5F, Font.NORMAL);
		Font font2 = new Font(Font.FontFamily.HELVETICA, 8.5F, Font.BOLD);
		Font font3 = new Font(Font.FontFamily.HELVETICA, 8.5F, Font.NORMAL);
		Font fontSerieYFolio = new Font(Font.FontFamily.HELVETICA, 9.5F, Font.BOLD);
		Font fontHead = new Font(Font.FontFamily.HELVETICA, 8.5F, Font.NORMAL);
		fontHead.setColor(BaseColor.WHITE);
		...
		String lugarFechaEmiHoraCert = comprobante.getLugarExpedicion().concat(" a ")
...
*/
	...
	public Document construirPdf(Comprobante comprobante)
			throws DocumentException, MalformedURLException, IOException {

		construirBoceto(comprobante, null);
		...
		// agregarChunkYNuevaLinea("Serie del certificado de emisor:
		// ".concat(comprobante.getNoCertificado()), font3, fraseDatosFiscales);
		// agregarChunkYNuevaLinea("Serie del certificado del SAT:
		// ".concat(tfd.getNoCertificadoSAT()), font3, fraseDatosFiscales);
		agregarChunkYNuevaLinea(
				"Régimen fiscal: ".concat(comprobante.getEmisor().getRegimenFiscal().get(0).getRegimen()), font3,
				fraseDatosFiscales);
	...
	public Document construirPdfCancelado(Comprobante comprobante, String selloDigital,
			byte[] bytesQRCode, String selloCancelacion, Date fechaCancelacion) throws MalformedURLException, DocumentException, IOException {
		...
		Phrase fraseSelloCancelacion = new Phrase();
		Chunk chunkEtqSelloCancel = new Chunk("Sello digital SAT (Cancelación): ", fontTituloSellos);
		Chunk chunkSelloCancel = new Chunk(selloCancelacion, fontContenidoSellos);
	...
	private void construirBoceto(Comprobante comprobante, TimbreFiscalDigital tfd) throws DocumentException, MalformedURLException, IOException {
		/*Font fontTituloSellos = new Font(Font.FontFamily.HELVETICA, 7.5F, Font.BOLD);
		Font fontContenidoSellos = new Font(Font.FontFamily.HELVETICA, 7.5F, Font.NORMAL);
		Font font2 = new Font(Font.FontFamily.HELVETICA, 8.5F, Font.BOLD);
		Font font3 = new Font(Font.FontFamily.HELVETICA, 8.5F, Font.NORMAL);
